...
| **Rules 17–20, which are NOT in that note** | `docs/…BuildRoadmap_*.md` §7 — a design-session file | ⚑ **Corrected 2026-08-13.** *Rule 18 — a measurement is validated against known answers before its number is used* — was cited here, withdrawn on 08-12 as a number I had invented, and **it exists: §7 of the Build Roadmap carries rules 17, 18, 19 and 20 beside a restatement of 1, 11b and 13–16.** *Two overlapping lists in two files, one of which a code session never opens.* **The withdrawal was right on the evidence available and wrong on the fact** — which is this section's own argument, arriving as an error rather than a warning |
