...
/**
 * Re-exported so a caller reading a `SinceResult` does not have to know the
 * sequence moved. **The ordering itself lives in `visitSequence.ts`** — §1d's
 * discontinuity display needs the same visit order, and two modules deriving one
 * independently is exactly the shape rule 4 forbids.
 */
...
export function outstandingSince(args: {
  ...
  // The visit order, from the one module that derives it.
  const seq = visitSequence(db, propertyId)
  const { visits: sequence, reachesBack: recordReachesBack, indexOf } = seq
