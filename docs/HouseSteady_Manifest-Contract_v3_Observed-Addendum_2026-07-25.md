...
4. **Language lint** — the config defines a layer with id `monitor`, label "Monitoring". Internal vocabulary today; if it ever reaches a client-facing render it collides with the Scope's banned-word discipline. David adjudicates whether the ban covers equipment/technical senses.
