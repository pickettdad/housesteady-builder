...
| **O6** | **How a 2 GB baseline reaches a hosted builder — an open design question, recorded now so it is not discovered at the wrong moment.** Hosting is already a launch requirement (AI Processing Decision §4), and today's answer is *the export is on the same machine as the builder*, which stops being true the day it is hosted. **The walk made the shape concrete:** 529 MB for a seven-zone partial walk, against CLAUDE.md §11's 1.5–2 GB for a full baseline. That does not travel over a form post, and it does not travel through a chat attachment — this session proved both, one by measurement and one by a policy denial. Whatever the answer is (direct-to-object-storage upload from the iPad, a sync client on the concierge's machine, physical transfer for the first houses), **it is a Field-app-to-builder interface question and therefore a contract question**, which makes it the same category as D3 and F8 rather than an ops detail. **Not now** — but it should be answered before hosting is built, not during. | Before hosting; informs the manifest contract |
