...
**The panoramic sweep per zone** is the fix to `well-pressure-tank` × 4, and it is the cheapest one available. Those four proposals happened because three batches each saw a tank and none could see the others; **a sweep is the mutual context that was missing**, and it is better than the canvas because the canvas is a floor plan and a sweep is what the room actually looks like. A few frames per zone against 508 MB is a rounding error. **And F-26 lands today** — a sweep with bearings gives the desk a frame of reference for every other photograph in the zone, which nothing currently does.
